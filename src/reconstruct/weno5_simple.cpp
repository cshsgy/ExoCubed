...
void FaceReconstruct::Weno5X1Simple(const int k, const int j, const int il, const int iu,
...
void FaceReconstruct::Weno5X2Simple(const int k, const int j, const int il, const int iu,
  const AthenaArray<Real> &w, AthenaArray<Real> &wl, AthenaArray<Real> &wr)
{
  for (int n=0; n<w.GetDim4(); ++n)
#pragma omp simd
...
void FaceReconstruct::Weno5X3Simple(const int k, const int j, const int il, const int iu,
  const AthenaArray<Real> &w, AthenaArray<Real> &wl, AthenaArray<Real> &wr)
{
  for (int n=0; n<w.GetDim4(); ++n)
#pragma omp simd
