...
namespace Weno3Coeff {
  ...
  double wght[2] = {RAT1*(1 + RAT1)/(1 + RAT1 + RAT1*RAT1), 1/(1 + RAT1 + RAT1*RAT1)};
...
inline Real interp_weno3x1(Real phim1, Real phi, Real phip1) {
  ...
  Real b1 = (phim1 - phi)*(phim1 - phi) + (3*phim1 - phi)*(phim1 - phi)*eps;
  for (int i = 0; i < 3; ++i)
    b1 += (beta1[0][i] + beta1[1][i]*eps)*eps*eps*pp1[i];

  Real pp2[3] = {phi*phi, phi*phip1, phip1*phip1};
  Real b2 = (phi - phip1)*(phi - phip1) - (phi - phip1)*(phi - 3*phip1)*eps;
  for (int i = 0; i < 3; ++i)
