...
BlockIndex::BlockIndex(int nk, int nj, int ni, int mygid, int mylid):
  is(nghost), ie(nghost + ni - 1),
  js(nghost), je(nghost + nj - 1),
  ks(nghost), ke(nghost + nk - 1),
  gid(mygid), lid(mylid),
...
BlockIndex::BlockIndex(MeshBlock *pmb):
  ...
  js(pmb->js), je(pmb->je),
  ks(pmb->ks), ke(pmb->ke),
  gid(pmb->gid), lid(pmb->lid),
  ncells1(pmb->ncells1), ncells2(pmb->ncells2), ncells3(pmb->ncells3)
{
