...
void check_eos_cons2prim(AthenaArray<Real> const& prim, int k, int j, 
                         int il, int iu) {
#ifdef ENABLE_GLOG
  for (int i = il; i <= iu; ++i) {
...
void fix_eos_cons2prim(MeshBlock *pmb, AthenaArray<Real> &prim,
  ...
  for (int i = il; i <= iu; ++i) {
    int ifix = il;
    for (; ifix <= iu; ++ifix) {
      if ((prim(IDN, k, j, ifix) < 0.) ||
          std::isnan(prim(IDN, k, j, ifix)) ||
          (prim(IPR, k, j, ifix) < 0.) ||
          std::isnan(prim(IPR, k, j, ifix))) {
        break;
      }
    }
    ...
    for (int i = ifix; i <= iu; ++i) {
      ...
      prim(IDN, k, j, i) = prim(IDN, k, j, ifix-1) * exp(grav * z / (Rd * temp));
      prim(IPR, k, j, i) = prim(IPR, k, j, ifix-1) * exp(grav * z / (Rd * temp));
    }
  }
}
...
void fix_implicit_cons(MeshBlock *pmb, AthenaArray<Real> &cons, 
  ...
  for (int k = kl; k <= ku; ++k)
    for (int j = jl; j <= ju; ++j) {
      ...
      for (; ifix <= iu; ++ifix) {
        if ((cons(IDN, k, j, ifix) < 0.) ||
            std::isnan(cons(IDN, k, j, ifix)) ||
            (cons(IEN, k, j, ifix) < 0.) ||
            std::isnan(cons(IEN, k, j, ifix))) {
          break;
        }
      }

      AirParcel air0 = AirParcelHelper::gather_from_conserved(pmb, k, j, ifix-1);
      air0.ToMoleFraction();
