...
void Decomposition::SyncNewVariables(AthenaArray<Real> const& w, int kl, int ku, int jl, int ju)
{
  ...
  if (has_bot_neighbor) {
    ...
    if (bblock.snb.rank != Globals::my_rank) { // MPI boundary
      ...
      MeshBlock *pbl = pmy_hydro->pmy_block->pmy_mesh->FindMeshBlock(bblock.snb.gid);
      std::memcpy(pbl->pimpl->pdec->wrecv_top_, wsend_bot_, sbot*sizeof(Real));
      std::memcpy(wrecv_bot_, pbl->pimpl->pdec->wsend_top_, sbot*sizeof(Real));
    }
  }
  ...
  if (has_top_neighbor) {
    ...
    if (tblock.snb.rank != Globals::my_rank) { // MPI boundary
      ...
      MeshBlock *pbl = pmy_hydro->pmy_block->pmy_mesh->FindMeshBlock(bblock.snb.gid);
      std::memcpy(pbl->pimpl->pdec->wrecv_bot_, wsend_top_, stop*sizeof(Real));
      std::memcpy(wrecv_top_, pbl->pimpl->pdec->wsend_bot_, stop*sizeof(Real));
    }
  }
}
  ...
  int kl, int ku, int jl, int ju, int il, int iu, NeighborBlock nb)
{
  ...
  if (nb.snb.rank != Globals::my_rank) { // MPI boundary
    ...
    MPI_Recv(buffer_, ssize, MPI_ATHENA_REAL, nb.snb.rank, tag, MPI_COMM_WORLD, &status);
#endif
  } else {  // local boundary
    // need to wait for the top boundary to finish
  ...
  int kl, int ku, int jl, int ju)
{
  ...
  if (has_top_neighbor) {
    if (tblock.snb.rank != Globals::my_rank) {
      ...
      MeshBlock *pbl = pmy_hydro->pmy_block->pmy_mesh->FindMeshBlock(tblock.snb.gid);
      std::memcpy(pbl->pimpl->pdec->buffer_, send_buffer_ + s1, s1*sizeof(Real));
    }
  }
}
...
#ifdef MPI_PARALLEL
  MPI_Comm comm_col;
  MPI_Comm_split(MPI_COMM_WORLD, color_[Globals::my_rank], Globals::my_rank, &comm_col);
