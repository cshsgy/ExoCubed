...
inline void IntegrateUpwards(AthenaArray<Real>& psf,
  ...
  if (!has_top_neighbor) {
    ...
    for (int k=kl; k<=ku; ++k)
      for (int j=jl; j<=ju; ++j) {
        ...
        psf_(k,j,ie+1) = w(IPR,k,j,ie)*exp(-grav*pco->dx1f(ie)/(2*RdTv));
      }
  } else
  ...
  for (int k = kl; k <= ku; ++k)
    for (int j = jl; j <= ju; ++j) {
      ...
      for (int i = is - NGHOST; i <= ie + NGHOST; ++i) {
        ...
        dsv = pow(psv, 1./entropy_(0,k,j))*exp(-entropy_(1,k,j)/entropy_(0,k,j));
  ...
  AthenaArray<Real> &wl, AthenaArray<Real> &wr, int k, int j, int il, int iu)
{
  ...
  for (int i = il; i <= iu; ++i) {
    ...
    Real dsf = pow(psf_(k,j,i), 1./entropy_(0,k,j))*exp(-entropy_(1,k,j)/entropy_(0,k,j));
