...
ImplicitSolver::ImplicitSolver(MeshBlock *pmb, ParameterInput *pin):
    has_bot_neighbor(false), has_top_neighbor(false),
...
void ImplicitSolver::FindNeighbors() {
  ...
  if (mydir_ == X1DIR) {
    if (pmb_->block_size.x1min > pmesh_->mesh_size.x1min)
      first_block = false;
    if (pmb_->block_size.x1max < pmesh_->mesh_size.x1max)
      last_block = false;
  } else if (mydir_ == X2DIR) {
    if (pmb_->block_size.x2min > pmesh_->mesh_size.x2min)
      first_block = false;
    if (pmb_->block_size.x2max < pmesh_->mesh_size.x2max)
      last_block = false;
  } else { // X3DIR
    if (pmb_->block_size.x3min > pmesh_->mesh_size.x3min)
      first_block = false;
    if (pmb_->block_size.x3max < pmesh_->mesh_size.x3max)
      last_block = false;
  }
