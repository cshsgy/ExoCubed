...
void Hydro::RiemannSolver(int const k, int const j, int const il, int const iu, 
    ...
    AthenaArray<Real> &flx, AthenaArray<Real> const& dxw)
{
  ...
  for (int i = il; i <= iu; ++i) {
    ...
    flx(ivx,k,j,i) = 0.5*(wli[IDN]*wli[ivx]*wli[ivx] + 0.5*wli[IDN]*wli[IDN]
                        + wri[IDN]*wri[ivx]*wri[ivx] + 0.5*wri[IDN]*wri[IDN]);
    flx(ivy,k,j,i) = 0.5*(wli[IDN]*wli[ivx]*wli[ivy] + wri[IDN]*wri[ivx]*wri[ivy]);
    flx(IVX,k,j,i) = 0.;
