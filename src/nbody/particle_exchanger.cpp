...
void ParticleBase::Transfer(MeshBlock const *pmb, int n) {
  ...
  for (auto &nb : pmb->pbval->neighbor) {
    ...
    int tag = MessageHelper::create_mpi_tag(pmb->lid, nb.bufid, "Particle");

    MPI_Probe(nb.snb.rank, tag, MPI_COMM_WORLD, &status);
    MPI_Get_count(&status, MPI_PARTICLE_DATA, &rsize);

    recv_buffer_[nb.bufid].resize(rsize);
    MPI_Irecv(recv_buffer_[nb.bufid].data(), rsize, MPI_PARTICLE_DATA,
